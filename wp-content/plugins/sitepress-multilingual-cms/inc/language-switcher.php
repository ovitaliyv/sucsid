...
class SitePressLanguageSwitcher {
  ...
  function language_selector_footer() {
    ...
    if (!empty($languages)) {
      ...
      foreach ($languages as $lang) {
        echo '    <li'.(isset($lang['hidden'])?' class="disabled" title="'.__('on the stage of filling', 'twentyeleven').'"':'').'>';
        if (!isset($lang['hidden'])) {
        ...
        if ($this->settings['icl_lso_flags'] || $this->footer_preview)
          echo '<img src="' . $lang['country_flag_url'] . '" alt="' . $lang['language_code'] . '" class="iclflag"';
        if (!$this->settings['icl_lso_flags'] && $this->footer_preview)
  ...
  function admin() {
    ...
    <li>
      ...
      <ul>
        ...
        if (empty($this->settings['icl_post_availability_position']) ||
        ...
        <li>
          <label>
            <?php _e('Text for alternative languages for posts', 'sitepress'); ?><br /><input type="text" name="icl_post_availability_text" value="<?php if (isset($this->settings['icl_post_availability_text'])) echo esc_attr($this->settings['icl_post_availability_text']); else _e('This post is also available in: %s', 'sitepress'); ?>" size="40" />
          </label>
          <br /><br />
        </li>
      </ul>
  ...
  function wp_nav_menu_items_filter($items, $args) {
    ...
    if ($abs_menu_id == $sitepress_settings['menu_for_ls']) {
      ...
      $items .= '<li class="menu-item menu-item-language menu-item-language-current" title="' . $languages[$sitepress->get_current_language()]['translated_name'] . '">';
      ...
      if (!empty($languages)) {
        ...
        foreach ($languages as $code => $lang) {
          $items .= '<li class="menu-item menu-item-language' . ((isset($lang['hidden'])) ? ' disabled' : '') . '" title="' . ((isset($lang['hidden'])) ? $lang['native_name'] . ' (' . __('on the stage of filling', 'twentyeleven') . ')' : $lang['native_name']) . '">';
          if (!isset($lang['hidden'])) {
